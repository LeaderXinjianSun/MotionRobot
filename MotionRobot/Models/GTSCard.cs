...
namespace MotionRobot.Models
{
    ...
    public static class GTSCard
    {
        ...
        /// <returns></returns>
        public static bool SetCrd4D(short cardNum, short crd, bool isAbs, short x, short y, short z, short a, double maxSpeed, double maxAcc)
        {
            try
            {
                gts.mc.TCrdPrm crdPrm;
        ...
        //    }
        //}
        public static void AxisLnXYMove4D(AxisParm XAxis, AxisParm YAxis, AxisParm ZAxis, AxisParm RAxis, List<M1Point> targets, short crd, double acc)
        {
            // 指令返回值
            short sRtn;
            int space;
            ...
            for (int i = 0; i < targets.Count; i++)
            {
                ...
                {
                    case 0://直线普通定位
                        sRtn = gts.mc.GT_LnXYZA(XAxis.CardNo, crd, (int)(targets[i].X / XAxis.Equiv), (int)(targets[i].Y / YAxis.Equiv), (int)(targets[i].Z / ZAxis.Equiv), (int)(targets[i].R / RAxis.Equiv), targets[i].Speed / XAxis.Equiv / 1000, _acc, 0, 0);
                        //// 查询返回值是否成功
                        //if (0 != sRtn)
                        //{
                        //    do
                        ...
                        break;
                    case 1://直线精确定位
                        sRtn = gts.mc.GT_LnXYZAG0(XAxis.CardNo, crd, (int)(targets[i].X / XAxis.Equiv), (int)(targets[i].Y / YAxis.Equiv), (int)(targets[i].Z / ZAxis.Equiv), (int)(targets[i].R / RAxis.Equiv), targets[i].Speed / XAxis.Equiv / 1000, _acc, 0);
                        break;
                    case 2://圆弧定位
